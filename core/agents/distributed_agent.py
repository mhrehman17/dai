...
class DistributedAgent:
    def __init__(self, agent_id: str, server_address: str, peers: List[str], 
        ...
        self.failed_heartbeats = {}  # Track failed heartbeats for each peer
    ...
    def _heartbeat(self):
        ...
        while self.is_active and self.current_iteration < self.iteration_limit:
            ...
            for peer in self.peers:
                success = random.choices([True, False], weights=[0.8, 0.2], k=1)[0]  # Adjusted success rate to 80%
                if success:
                    ...
                    if self.failed_heartbeats[peer] > 2:
                        self.logger.warning(f"Failed to send heartbeat to peer {peer} (attempts: {self.failed_heartbeats[peer]})")
